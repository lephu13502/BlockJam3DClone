...
public class ObjectManager
{
    ...
    //create new method to create character with prefab, not primitive type
    public void CreateCharactersWithPrefab<CostumComponent>(List<GameObject> list, GameObject parentObject, GameObject prefab) where CostumComponent : Component
    {
        GameObject newObject;
    ...
    public CostumType AllocateObject<CostumType>(Node node, Color color, bool active = true)
    {
        ...
        if (typeof(CostumType) == typeof(Character))
        {
            _gObject.transform.position = node.worldPosition;
            //set animator to false
            _gObject.GetComponent<Animator>().SetBool("isWalking", false);
            Transform ninjaChild = _gObject.transform.Find("Ninja");
            if (ninjaChild != null)
            {
